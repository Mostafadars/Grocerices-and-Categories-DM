...
class CategoriesMinerLogic:
    ...
    def load_data(self, file_path: str, percentage: float = 100.0):
        self._log_progress(f"Loading data from {file_path}...")
        ...
        self.transactions = []
        for line in lines:
            categories = [cat.strip() for cat in line.strip().split(';') if cat.strip()]
            if categories:
                self.transactions.append(categories)
        
        self._log_progress(f"Loaded {len(self.transactions)} transactions")
        
        if percentage < 100.0:
            num_transactions = int(len(self.transactions) * percentage / 100.0)
            self.transactions = random.sample(self.transactions, num_transactions)
            self._log_progress(f"Using {len(self.transactions)} transactions ({percentage}% of data)")
        
        return self.transactions
    
    def create_vertical_format(self, transactions: List[List[str]]):
        ...
        self._log_progress(f"Created vertical format with {len(self.vertical_data)} unique categories")
        return self.vertical_data
    
    def apriori_vertical(self, vertical_data: Dict[str, set], min_support_count: int):
        """Apriori algorithm implementation using vertical format"""
    ...
    def generate_association_rules(self, itemsets: List[Tuple[frozenset, int]], min_confidence: float, 
                                 all_frequent_itemsets: List[Tuple[frozenset, int]]):
                ...
                for antecedent in combinations(items, i):
                    ...
                    if antecedent_support > 0:
                        ...
                        if confidence >= min_confidence:
                            ...
                            lift = confidence / (consequent_support / len(self.vertical_data)) if consequent_support > 0 else float('inf')
    ...
    def generate_candidates(self, prev_itemsets: List[frozenset], level: int):
        candidates = set()
    ...
    def get_results_text(self):
        ...
        self.frequent_itemsets.sort(key=lambda x: x[1], reverse=True)
        ...
        for size in sorted(itemsets_by_size.keys()):
            itemsets_text += f"Frequent {size}-itemsets:\n"
            itemsets_text += "-"*20 + "\n"
        ...
        self.association_rules.sort(key=lambda x: x[3], reverse=True)
